...
int main(){
	...
	for(const auto& pair : guard_map){
		...
		for(int i = 0;i<60; i++)
			if(sleeps[i] > max_sleep_freq){
				max_sleep_min = i;
				max_sleep_id = id;
				max_sleep_freq = sleeps[i];
			}
	}
	
	max_sleep_min++;

	std::cout << "ID: " << max_sleep_id << ", TIME: " << max_sleep_min << ", ANS: " << max_sleep_id* max_sleep_min << std::endl;
}
